...
tracked in the
[`perfaud MVP Completion Plan`](mvp_plan.md), and
broader Audit direction is governed by the
[`perfaud Product Constitution`](product_constitution.md) and
[`perfaud Roadmap`](roadmap.md).
...
- Replace existing `perfaud.axys_apx` loading behavior.
...
match stripped source text by exact case. perfaud does not infer meaning from code
...
run settings together in `perfaud.yaml`. One file points at both snapshot
...
a partial overlay on the common mappings in `perfaud.yaml`.
...
maps both `security_type` and `security_symbol`, perfaud constructs the compact
...
inputs: perfaud constructs the existing `security_id` and does not add the
...
from Python. The user-facing `perfaud run` orchestrator explicitly selects
...
2. Relative paths in `perfaud.yaml` resolve relative to that Audit YAML file.
...
from perfaud's normalized field name. Omitting a mapping accepts only the exact
normalized name, including case; perfaud does not guess vendor aliases.

Column mappings should resolve in this order:

1. Complete external schema referenced by that snapshot, when configured.
2. Common `files.<dataset>.columns` mapping in the Audit `perfaud.yaml` when no
...
- `perfaud` exposes selected workflow and report entrypoints; explanation
...
perfaud does not add `local_` duplicates because row currency is already the

...
for Modified Dietz formula roles without turning perfaud into a local accounting
system.
...
can therefore have different explicitly reviewed meanings. perfaud does not infer
meaning from transaction code alone, so an unmatched code remains unknown and
uppercase does not mean cancellation. When `extract_contract` is omitted, perfaud
...
conditional rules before perfaud treats an ambiguous code as an external flow,
...
The check reports absence only. Zero cost counts as supplied, and perfaud neither
...
future versioned Axys/APX Audit profile. perfaud keeps these as tested candidate
...
context for these profiles, not requirements for perfaud's Modified Dietz
...
interpret code-only ambiguous rows outside perfaud's extract contract. It should be
rare and explicit:

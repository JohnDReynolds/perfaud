...
This note sketches a perfaud-normalized Axys/APX extract shape for analytics and
...
and `secperf.csv` are perfaud-normalized filenames, not verified native object or
profile names.
...
| perfaud dataset | Practical first source | Local questions to resolve |
| --- | --- | --- |
...
- [generated extract requirements](contracts/demo_extract_availability.md) for
  perfaud dataset requirements and likely IMEX/REP source paths;
- [transaction semantics YAML](contracts/transaction_semantics_matrix.yaml) for
  executable perfaud transaction treatment; and

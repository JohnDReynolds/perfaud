...
[perfaud Axys/APX Extract Requirements and Source Guidance](../axys_apx/contracts/demo_extract_availability.md).
...
The comparison YAML must identify the source datasets perfaud is allowed to read
...
unprefixed. perfaud does not create parallel `local_` names because row currency
...
Axys/APX starter, perfaud constructs this key from exact source columns `Security
...
characters, but perfaud does not impose a length restriction. A reference-qualified
...
perfaud stops before producing a report. Optional datasets may be omitted; if a
...
value, and dated external cash flows. It does not need perfaud to rebuild an
...
before perfaud can classify them: margin or bond/principal context, cash movement,
...
`src/perfaud/templates/axys_apx/demo_extract_availability.yaml`. A site comparison YAML

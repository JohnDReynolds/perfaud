...
The canonical implementation lives under `perfaud.data_issues`:
...
reference snapshot. This is a perfaud input contract, not a claim that symbol alone
...
separator such as `_`. perfaud rejects blank or whitespace-padded components and
ambiguous constructed values rather than guessing between duplicate symbols. Axys/APX
types are typically four characters, but perfaud preserves the source value instead
...
4. `holdings_stale_price`: within each snapshot, portfolio, and security in an
   ...
   perfaud did not observe every intervening day.

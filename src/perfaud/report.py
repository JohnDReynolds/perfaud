...
from perfaud import atomic_directory as _atomic_directory
...
from perfaud import runner as _pc_runner
from perfaud.workbook import writer as _pc_workbook
from perfaud.workbook import layout as _pc_workbook_layout
...
from perfaud.specification import PORTFOLIO_COMPARISON_LEVEL
...
_REVIEW_KEY = _review.REVIEW_KEY
...
_OPTIONAL_REPORT_BUNDLE_TABLE_ARTIFACTS = (
    _review.RECONSTRUCTION_SUMMARY_ARTIFACT,
    _review.RETURN_RECONSTRUCTION_CHECKS_ARTIFACT,
    _review.SECURITY_RETURN_RECONSTRUCTION_CHECKS_ARTIFACT,
)
...
_period_key = _review.period_key
_period_review_key = _review.period_review_key
_row_review_key = _review.row_review_key
...
def _html_workbook_sheet_section(sheet: _pc_workbook.ReviewWorkbookSheet) -> str:
    ...
    if sheet.artifact_name == _review.EXECUTIVE_SUMMARY_ARTIFACT:
...
def _html_executive_summary_tables(
    ...
    for section_name, caption in (
        ...
        headers = "".join(
            f'<th scope="col"{_html_title_attribute(_review.audit_term_tooltip(header))}>'
...
def _html_workbook_body_cell(
    ...
    *,
    ...
    title_attribute = _html_title_attribute(
        _review.audit_value_tooltip(column, value)
    )
...
def write_report_bundle(
    ...
    *,
    ...
    Raises:
        PerfaudError: If report construction, validation, or promotion fails.
    ...
    *,
    ...
    Args:
        ...
        require_complete_yaml_setup: Whether every changed source-data field
            that perfaud knows how to classify must have explicit additive,
    ...
    Raises:
        PerfaudError: If report validation fails.
    """
    ...
    cause_sheet = next(
        ...
        == _review.PERFORMANCE_DIFFERENCE_CAUSES_ARTIFACT
    )
    tables[_review.CAUSE_LINEAGE_ARTIFACT] = _cause_lineage_export_table(
        cause_sheet.table
    )
    for sheet in workbook_sheets:
    ...
    paths: dict[str, Path] = {}
    html_report_file_name = _review.html_report_file_name(comparison_level)
    review_workbook_file_name = _review.review_workbook_file_name(
        comparison_level
    )
    ...
    *,
    ...
    if include_reconstruction_diagnostics:
        ...
        if not reconstruction_summary.is_empty():
            tables[_review.RECONSTRUCTION_SUMMARY_ARTIFACT] = (
                reconstruction_summary
            )
        if not reconstruction_checks.is_empty():
            tables[_review.RETURN_RECONSTRUCTION_CHECKS_ARTIFACT] = (

...
_IDENTITY_SECURITY_SYMBOL_COLUMN = "__perfaud_identity_security_symbol"
_IDENTITY_SECURITY_TYPE_COLUMN = "__perfaud_identity_security_type"
...
_CONSTRUCTED_SECURITY_ID_COLUMN = "__perfaud_constructed_security_id"
...
def csv_to_internal_mappings(
    ...
    Raises:
        PerfaudError: If a required normalized column is missing or if any
...
def read_mapped_csv(
    ...
    Raises:
        PerfaudError: If column mappings cannot be resolved unambiguously.
    """
...
def require_numeric_columns(
    ...
    *,
    ...
    Raises:
        PerfaudError: If a present numeric column contains a nonblank value that
...
def snapshot_by_key(
    ...
    raise PerfaudError(f"Unknown snapshot key: {snapshot_key}")
...
def optional_file_path(
    ...
    Raises:
        PerfaudError: If ``snapshot_key`` is not a known neutral snapshot key.
    """
...
def aliases_with_schema_overrides(
    ...
    Raises:
        PerfaudError: If a referenced schema YAML cannot be parsed or has invalid

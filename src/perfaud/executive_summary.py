...
from perfaud.data_issues.vocabulary import DataIssueType
...
def executive_summary_table(
    ...
    *,
    ...
    Raises:
        PerfaudError: If a review status, issue type, or comparison level is unknown.
    """
...
def _primary_performance_frames(
    ...
    if snapshot_a is None or snapshot_b is None:
        raise PerfaudError("Security performance input is unavailable.")
...
def _performance_row(label: str, statuses: Sequence[str]) -> dict[str, object]:
    ...
    if bucket_total != len(statuses):
        raise PerfaudError("Executive Summary performance quantities do not reconcile.")
    return row
...
def _assert_supported_level(comparison_level: str) -> None:
    ...
    if comparison_level not in {
        ...
        raise PerfaudError(f"Unsupported comparison level: {comparison_level!r}")

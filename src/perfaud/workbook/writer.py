...
REVIEW_WORKBOOK_ARTIFACT = _review.REVIEW_WORKBOOK_ARTIFACT
...
PRIMARY_DIFFERENCE_SHEETS = _review.PRIMARY_REVIEW_SHEETS
SHARED_REVIEW_SHEETS = _review.SHARED_REVIEW_SHEETS
EXPECTED_SHEETS = _review.EXPECTED_REVIEW_SHEETS
REQUIRED_HEADERS = {
    _review.PERFORMANCE_DIFFERENCES_SHEET: (
        "Portfolio",
        "From Date",
        "Thru Date",
    ...
    _review.PERFORMANCE_DIFFERENCE_CAUSES_SHEET: (
        "Portfolio",
        "From Date",
        "Thru Date",
        "As Of Date",
    ...
    _review.TRANSACTION_MATCHING_DIAGNOSTICS_SHEET: (
...
def ensure_openpyxl_installed() -> None:
    ...
    Raises:
        PerfaudError: If ``openpyxl`` is not installed.
    """
    _load_openpyxl()
...
def _assert_written_portfolio_explanations_reconcile(workbook: Any) -> None:
    ...
    Raises:
        PerfaudError: If the values that will be written to the two reviewer-facing
            worksheets no longer foot at the workbook's six-decimal precision.
    """
    summary_name = _review.PERFORMANCE_DIFFERENCES_SHEET
    causes_name = _review.PERFORMANCE_DIFFERENCE_CAUSES_SHEET
...
def _add_workbook_sheet(
    ...
    for column_name, cell in zip(columns, worksheet[1]):
        ...
        cell.comment = styles["comment_class"](column_tooltip(column_name), "perfaud")
...
def _write_executive_section(
    ...
    *,
    ...
    for column_number, displayed_header in enumerate(displayed_headers, start=1):
        ...
        tooltip = _review.audit_term_tooltip(headers[column_number - 1])
        if tooltip:
            cell.comment = styles["comment_class"](tooltip, "perfaud")
...
def _append_and_format_workbook_data_rows(
    ...
    data_issues_explanation_column = (
        ...
        if worksheet.title == _review.DATA_ISSUES_SHEET
        else None
    )
...
def _format_workbook_columns(
    ...
    for column_index, (column_name, header, max_width) in enumerate(
        ...
        if (
            worksheet.title == _review.DATA_ISSUES_SHEET
...
def _review_workbook_sheet_issues(workbook: Any, artifact_file: str) -> list[str]:
    ...
    if not sheet_names or sheet_names[0] != _review.EXECUTIVE_SUMMARY_SHEET:
        issues.append(
            f"{artifact_file} must begin with primary sheet "
            f"{_review.EXECUTIVE_SUMMARY_SHEET!r}"
        )
    if _review.PERFORMANCE_DIFFERENCES_SHEET not in sheet_names:
        issues.append(
            f"{artifact_file} is missing primary sheet "
            f"{_review.PERFORMANCE_DIFFERENCES_SHEET!r}"
        )
    ...
    for sheet_name in (*PRIMARY_DIFFERENCE_SHEETS, *required_sheets):
        if sheet_name not in sheet_names:
            ...
            continue
        if sheet_name == _review.EXECUTIVE_SUMMARY_SHEET:
            issues.extend(

...
def _html_review_parity_issues(
    ...
    artifact_file = artifacts.get(_review.HTML_REPORT_ARTIFACT)
    if not isinstance(artifact_file, str):
        return []
    ...
    if (
        ...
        or parser.section_order[0] != _review.EXECUTIVE_SUMMARY_SHEET
    ):
        issues.append(
    ...
    for artifact_name, raw_metadata in review_sheets.items():
        ...
        if sheet_name not in parser.section_titles:
            ...
            continue
        if artifact_name == _review.EXECUTIVE_SUMMARY_ARTIFACT:
            actual_payload = {
...
def _xlsx_review_parity_issues(
    ...
    artifact_file = artifacts.get(_review.REVIEW_WORKBOOK_ARTIFACT)
    ...
    if (
        ...
        or workbook.sheetnames[0] != _review.EXECUTIVE_SUMMARY_SHEET
    ):
        issues.append(

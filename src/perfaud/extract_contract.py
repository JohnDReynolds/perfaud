...
from perfaud.axys_apx.transaction_safety import AMBIGUOUS_FLOW_TRANSACTION_CODES
import perfaud.paths as util

_CONTRACT_RESOURCE: Final[str] = "perfaud"
_CONTRACT_RESOURCE_DIRECTORY: Final[tuple[str, ...]] = ("templates", "axys_apx")
...
def validate_extract_contract(
    ...
    *,
    ...
    Raises:
        PerfaudError: If the contract is structurally invalid, omits transaction
...
def extract_contract_settings(
    ...
    *,
    ...
    Raises:
        PerfaudError: If ``extract_contract`` has an invalid shape, contains an
...
def validate_transaction_extract_contract(
    ...
    *,
    ...
    Raises:
        PerfaudError: If ambiguous Axys/APX transaction codes are present and the
...
def _packaged_contract_label() -> str:
    ...
    return (
        f"packaged:{_CONTRACT_RESOURCE}/{'/'.join(_CONTRACT_RESOURCE_DIRECTORY)}/"
...
def _load_local_extract_contract(path: str) -> dict[str, Any]:
    ...
    if not isinstance(loaded, dict):
        raise PerfaudError(f"Extract contract {path!r} must contain a YAML mapping.")
    return loaded
...
def _required_mapping(
    ...
    if not isinstance(value, dict):
        raise PerfaudError(f"{contract_label}: {key} must be a mapping.")
    return value
...
def _normalized_transaction_column(demo_column: str) -> str:
    ...
    raise PerfaudError(f"Unsupported contract transaction column {demo_column!r}.")

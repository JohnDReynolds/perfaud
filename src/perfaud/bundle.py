...
from perfaud import extract_contract as _pc_extract_contract
from perfaud import lineage as _pc_lineage
from perfaud import output_integrity as _pc_output_integrity
...
from perfaud.portfolio_performance import SnapshotKey
...
REPORT_BUNDLE_REQUIRED_ARTIFACTS = (
    ...
    "findings",
    "source_detail",
    _review.EXECUTIVE_SUMMARY_ARTIFACT,
    _review.PERFORMANCE_DIFFERENCES_ARTIFACT,
    _review.PERFORMANCE_DIFFERENCE_CAUSES_ARTIFACT,
    _review.DATA_ISSUES_ARTIFACT,
    _review.CAUSE_LINEAGE_ARTIFACT,
...
_REVIEW_VOCABULARY = {
    "formula_input": (
        "A value that perfaud can use directly in the Modified Dietz return "
...
def report_bundle_contract() -> dict[str, object]:
    ...
    return {
        ...
        "audit_artifact_files": {
            comparison_level: {
                _review.HTML_REPORT_ARTIFACT: (
                    _review.html_report_file_name(comparison_level)
                ),
                _review.REVIEW_WORKBOOK_ARTIFACT: (
                    _review.review_workbook_file_name(comparison_level)
                ),
            }
        ...
        "primary_review_artifact_modes": {
            "xlsx": _review.REVIEW_WORKBOOK_ARTIFACT,
            "html": _review.HTML_REPORT_ARTIFACT,
...
def write_report_bundle_readme(
    ...
    *,
    ...
    Returns:
        Normalized destination path.
    """
    primary_sheet = _review.EXECUTIVE_SUMMARY_SHEET
    ...
    causes_review_artifact = (
        ...
        if csv_only_output
        else _review.PERFORMANCE_DIFFERENCE_CAUSES_SHEET
    )
    performance_review_artifact = (
        "`performance_differences.csv`"
        if csv_only_output
        else _review.PERFORMANCE_DIFFERENCES_SHEET
    )
    data_issues_review_artifact = (
        "`data_issues.csv`"
        if csv_only_output
        else _review.DATA_ISSUES_SHEET
    )
    lines = [
...
def _report_bundle_review_entrypoints(
    ...
    *,
    ...
    primary_review: object = artifacts.get(
        _review.REVIEW_WORKBOOK_ARTIFACT,
        artifacts.get(_review.HTML_REPORT_ARTIFACT),
    )
...
def _report_bundle_output_mode_issues(
    ...
    for option_name, artifact_name in (
        ("include_workbook", _review.REVIEW_WORKBOOK_ARTIFACT),
        ("include_html_output", _review.HTML_REPORT_ARTIFACT),
    ):
...
def _csv_table_validation_issues(
    ...
    if table_name == _review.CAUSE_LINEAGE_ARTIFACT:
...
def _report_bundle_lineage_issues(bundle_path: Path, artifacts: Mapping[str, object]) -> list[str]:
    ...
    cause_file = artifacts.get(_review.CAUSE_LINEAGE_ARTIFACT)

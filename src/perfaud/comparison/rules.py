...
from perfaud.comparison.findings import Finding
...
class SuppressionRule:
    ...
    def from_mapping(cls, values: object, index: int) -> "SuppressionRule":
        ...
        Raises:
            PerfaudError: If the suppression entry is not a mapping or does not
                contain a string ``code``.
        """
        if not isinstance(values, dict):
            raise PerfaudError(f"suppressions[{index}] must be a mapping.")
        code = values.get(_CODE_KEY)
        if not isinstance(code, str) or not code:
            raise PerfaudError(f"suppressions[{index}].code must be a string.")
...
def suppression_rules(
    ...
    Raises:
        PerfaudError: If the ``suppressions`` section has an invalid shape.
    """
    values = specification.values.get(_SUPPRESSIONS_KEY, [])
    if values is None:
        return []
    if not isinstance(values, list):
        raise PerfaudError("suppressions must be a list.")
    return [
